...
def all_score_export():
    ...
    for i in range(int(score_num)):
        print(f"{type_num+1}つ目の楽譜「{score_name}_{score_type[type_num]} score」を処理中…")
...
def valueClickTyping(x_position, y_position, value):
...
def pageMargin(howManyTimes=1):
    ...
    if howManyTimes == 1:
        ...
        # 適応対象→1ぺージ目のみ
        pyautogui.click(63, 250, 1, 0, "left")
        time.sleep(0.1)
        tabPress(1)
        valueTyping(1)
        print(howManyTimes)
        ...
        # 適応対象→適応するページの範囲
        pyautogui.click(61, 323, 1, 0, "left")
        tabPress(2)
        # から
        valueTyping(2)
        print(howManyTimes)
...
button_2 = tkinter.Button(root, text="クローズハット", command=lambda: inst_replace(2))
button_2.place(x=5, y=30)
button_3 = tkinter.Button(root, text="オープンハット", command=lambda: inst_replace(3))
button_3.place(x=80, y=30)
button_4 = tkinter.Button(root, text="ハイハットペダル", command=lambda: inst_replace(4))
button_4.place(x=160, y=30)
button_5 = tkinter.Button(root, text="ライドシンバル", command=lambda: inst_replace(5))
...
button_10 = tkinter.Button(root, text="ハイタム", command=lambda: inst_replace(10))
button_10.place(x=5, y=120)
button_11 = tkinter.Button(root, text="ハイミッドタム", command=lambda: inst_replace(11))
button_11.place(x=80, y=120)
button_12 = tkinter.Button(root, text="ローミッドタム", command=lambda: inst_replace(12))
button_12.place(x=160, y=120)
button_13 = tkinter.Button(root, text="ロータム", command=lambda: inst_replace(13))
button_13.place(x=5, y=150)
button_14 = tkinter.Button(root, text="フロアタム1", command=lambda: inst_replace(14))
button_14.place(x=80, y=150)
button_15 = tkinter.Button(root, text="フロアタム2", command=lambda: inst_replace(15))
button_15.place(x=160, y=150)
button_16 = tkinter.Button(root, text="クローズド・リムショット", command=lambda: inst_replace(16))
button_16.place(x=160, y=0)
button_17 = tkinter.Button(root, text="ライドベル", command=lambda: inst_replace(17))
button_17.place(x=160, y=90)
button_18 = tkinter.Button(root, text="スネア（ゴースト・ノート）", command=lambda: inst_replace(18))
...
# --------------------
button_ue = tkinter.Button(root, text="符頭を上向きに", command=lambda: futouMuki("u"))
button_ue.place(x=5, y=200)
button_shita = tkinter.Button(root, text="符頭を下向きに", command=lambda: futouMuki("d"))
...
button_rest = tkinter.Button(root, text="休符を移動する", command=rest)
...
button_kashi = tkinter.Button(root, text="歌詞を入力する", command=kashi)
button_kashi.place(x=5, y=310)

button_kashi_delete = tkinter.Button(root, text="歌詞を削除する", command=kashi_delete)
button_kashi_delete.place(x=120, y=310)
# --------------------
button_hand_MIDI = tkinter.Button(root, text="ドラムMIDIの下ごしらえ（手）", command=hand_MIDI)
...
button_score_export = tkinter.Button(root, text="楽譜を書き出す", command=score_export)
...
button_title_delete = tkinter.Button(root, text="タイトルを削除する", command=title_delete)
...
lbl_bottom = tkinter.Label(text="ボトムのマージンを設定する：　　　　　※8弦(0.64) , ドラム(0.38) , ベース(0.42)")
...
button_page_margin = tkinter.Button(root, text="ページマージンを調整する", command=page_margin)
button_page_margin.place(x=140, y=510)

button_page_margin = tkinter.Button(root, text="ペースト対象項目ウインドウ", command=paste)
button_page_margin.place(x=10, y=550)

button_page_margin = tkinter.Button(root, text="FinaleScriptパレット", command=FinaleScript)
